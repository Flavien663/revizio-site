...
export function StoreButtons({ size = "md", layout = "row" }: Props) {
  const isLg = size === "lg";
  const base =
    "inline-flex items-center gap-3 rounded-xl transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background";
  const dimensions = isLg ? "px-6 py-4" : "px-5 py-3";
  ...
  return (
    ...
    >
      ...
      <a
        ...
        className={`${base} ${dimensions} border border-border bg-surface text-text-primary hover:border-text-secondary`}
      >
